defmodule Peepchat.RoomControllerTest do
  ...
  @valid_data %{data: %{type: "rooms", attributes: @valid_attrs, relationships: %{}}}
  @invalid_attrs %{}
  @invalid_data data: %{type: "rooms", attributes: @invalid_attrs, relationships: %{}}
  ...
  test "shows chosen resource", %{conn: conn, user: user} do
    room = Repo.insert! %Room{name: "a room", owner_id: user.id}
    ...
    assert json_response(conn, 200)["data"] == %{
      ...
      "relationships" => %{
        "owner" => %{
          "links" => %{
            "related" => user_url(Endpoint, :show, user)
          }
        }
      }
    }
  ...
  test "creates and renders resource when data is valid", %{conn: conn, user: _user} do
    conn = post conn, room_path(conn, :create), @valid_data
  ...
  test "does not create resource and renders errors when data is invalid", %{conn: conn, user: _user} do
    conn = post conn, room_path(conn, :create), @invalid_data
  ...
  test "updates and renders chosen resource when data is valid", %{conn: conn, user: user} do
    ...
    conn = put conn, room_path(conn, :update, room),
      data: %{id: room.id, type: "rooms", attributes: @valid_attrs}
  ...
  test "does not update chosen resource and renders errors when data is invalid", %{conn: conn, user: user} do
    ...
    conn = put conn, room_path(conn, :update, room),
      data: %{id: room.id, type: "rooms", attributes: @invalid_attrs}
    assert json_response(conn, 422)["errors"] != %{}
  end
